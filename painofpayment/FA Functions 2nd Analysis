...
reg_results = function(factor_data, y, x_vars, prod_pooled, id) {
  ...
  if (prod_pooled) {
    # ---- OLS regression: used for pooled/aggregated data (e.g. one row per product) ----

    reg_formula <- as.formula(paste(y, "~", paste(x_vars, collapse = " + ")))
    model <- lm(reg_formula, data = factor_data)

    coef_table <- coef(summary(model))
    colnames(coef_table)[colnames(coef_table) == "Pr(>|t|)"] <- "p_value"
    ...
    model <- clmm(ordinal_formula, data = factor_data,
    ...
    null_model <- clmm(null_formula, data = factor_data,
                        control = clmm.control(maxIter = 1000, gradTol = 1e-6, maxLineIter = 100))
    r2_value <- as.numeric(1 - logLik(model) / logLik(null_model))
    r2_type  <- "mcfadden_r2"
