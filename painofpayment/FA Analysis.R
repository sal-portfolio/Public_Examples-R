...
ind_vars <- c(colnames(fa_prod1$scores), "like_1", "value_1", "ln_disutility")
drop <- c("f_expect_free", "f_overpriced")
ind_vars <- ind_vars[!(ind_vars %in% drop)]
reg_RE_prod1fa <- reg_results(df_long_scored,"painful_1", 
ind_vars, FALSE, "ResponseId")
...
write.csv(FA_onProd1_output, "painofpayment/output/final_likeval_disutility_numonly.csv", row.names = FALSE, na = "")
